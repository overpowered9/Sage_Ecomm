...
const visited_products = async (req, res) => {
  if (
    ...
    req.session.visitedProducts.length === 0
  ) {
  ...
  for (const item of cart) {
    try {
      ...
      const sizeObj = product.sizes.find(
        (sizeObj) => sizeObj.size === parseInt(item.size)
      );
      if (!sizeObj) {
        console.error(
          `Size ${item.size} not found for product with ID ${item.productId}`
        );
        continue;
      }
      ...
      if (sizeObj.quantity >= item.quantity) {
        ...
        console.error(
          `Not enough stock for size ${item.size} of product with ID ${item.productId}`
        );
      }
    } catch (error) {
      console.error(
        `Error processing product with ID ${item.productId}:`,
        error
      );
    }
  }
...
const updateCartItem = (req, res) => {
  ...
  const productIndex = cart.findIndex(
    (item) => item.productId === productId && item.size === size.toString()
  );
  if (productIndex !== -1) {
